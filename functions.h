...
void Convert_User_to_NDC(TPad* iPad, Double_t x_User, Double_t y_User, Double_t &x_NDC, Double_t &y_NDC)
{
  ...
  std::cout << "X1: " << iPad->GetX2() << ", X2: " << iPad->GetX2() << std::endl;
}
//------------------------------------------------------------------------------------------------------------
                           ...
                           Int_t FillColor, Float_t x_start, Float_t x_stop, Float_t transparency)
{
  ...
  Int_t N_total_points = N_points*2+1;
  std::cout << "N_total_points = " << N_total_points << std::endl;
  TGraph* Hist_line       = new TGraph(N_total_points);
  TGraph* Hist_line_upper = new TGraph(N_points);
  TGraph* Hist_line_lower = new TGraph(N_points);
  Hist_line ->SetLineWidth(LineWidth);
...
Double_t Get_muB(Double_t sqrt_sNN, Int_t flag_input, Double_t &muB_err)
{
  ...
  if(!(flag_input == 0 || flag_input == 1 || flag_input == 2))
    {
      std::cout << "WARNING: Get_muB function, flag_input is wrong. Either 0 or 1. Parametrization is used." << std::endl;
      flag_input = 0;
    }
...
{
  std::cout << "Set basic ROOT graphics style" << std::endl;
  //gStyle->Reset();
  gStyle->SetPalette(1);
...
{
  // calculates the minimum distance std::vector of a point to a straight given as parametric straight x = base + n * dir
		      ...
		      )
{
  ...
  std::vector<TPolyLine*> tp_Circles_lines;
  ...
  for(Int_t iFeed_down = 0; iFeed_down < N_feed_down_channels; iFeed_down++)
    {
...
#if 1
      std::cout << "iFeed_down = " << iFeed_down << ", PID_mother = " << PID_mother << ", PID_daughterA = " << PID_daughterA  << ", PID_daughterB = " << PID_daughterB
	  ...
	  if (iSample != 0  &&  iSample % 2000 == 0)
	      ...
	      std::cout << " " << iSample << " (" << event_percent << "%) " << "\n" << "==> Processing data, " << std::flush;
            }
  ...
  for(Int_t iPoint = 0; iPoint < tgae_output_v2->GetN(); iPoint++)
    {
      ...
      std::cout << "iPoint = " << iPoint << ", y_val_FD = " << y_val_FD << ", y_val_in = " << y_val << ", y_val_out = " << y_val_out
		<< ", input_weigh_factor = " << input_weigh_factor << ", total_weigh_factor = " << total_weigh_factor << std::endl;
    }
...
class Calc_syst_error_graph_hist
{
  ...
  std::vector<TGraphAsymmErrors*> vec_tgae;
  std::vector<TH1F*> vec_hist;
  ...
  if(RMS[0] >= 0.0)
    {
...
#if 0
	      std::cout << "i_MC: " << i_MC << ", RMS_above_below: " << RMS_above_below << ", val_nom: " << val_nom << ", nom_err: " << nom_err << ", val_den: " << val_den << ", den_err: " << den_err
			<< ", ratio_val: " << ratio_val << ", ratio_nom_den (mean): " << ratio_nom_den << ", N_ratios: " << N_ratios
			<< ", RMS_val: " << RMS[RMS_above_below]  << std::endl;
#endif
            }
        }
	  ...
	  if(vec_RMS[i_above_below].size() > 0)
	      ...
	      RMS[i_above_below] = TMath::Sqrt((1.0/((Double_t)vec_RMS[i_above_below].size()))*RMS[i_above_below]);
            ...
            {
	      RMS[i_above_below] = -1;
	      std::cout << "WARNING, RMS std::vector size below 1, old RMS used" << std::endl;
            }
        }
    }
...
{
  // Calculate normalized ray std::vector
...
{
  std::vector<TGraphAsymmErrors*> vec_tgae; // input graphs
...
{
  // Calculated the B-field "B_fieldvector" at position "B_pos" for a infinitesimal small conductor with length "dl" and current "current"
  TVector3 B_fieldvector = dl.Cross(B_pos);
  Double_t scalar_constant = (mu_zero/(4.0*TMath::Pi()))*current/TMath::Power(B_pos.Mag(),3.0);
  B_fieldvector *= scalar_constant;
...
//----------------------------------------------------------------------------------------
Int_t getdir (std::string dir, std::vector<std::string> &files)
{
  DIR *dp;
  struct dirent *dirp;
...
TH1D* Project_h2D::get_projection(TString name)
{
  ...
  // Define std::vector from start to stop
  ...
  // Calculate slope and offset, check if the std::vector is going exactly into y-direction
  Int_t flag_perp = 0;
  Double_t m, b;
...
TH2D* Subtract_plane_from_h2D(TH2D* h2D_in, TVector3 vec_x, TVector3 vec_y, Double_t min_elevation_ext)
{
  ...
  std::cout << "Plane subtracted" << std::endl;
  return h2D_in_sub;
}
//----------------------------------------------------------------------------------------
...
void Calc_radial_distribution::calc_radial_distribution()
{
  ...
  //-------------------------------------------------------
  if(flag_input_data == 1) // std::vector data input
    {
      std::cout << "data size: " << vec_data.size() << std::endl;
  ...
  if(flag_input_data == 2) // TPolyMarker3D data input
    {
      std::cout << "data size: " << TPM3D_data->GetN() << std::endl;
...
void fit_line_to_points()
{
  ...
  std::cout << "bestX: " << bestX << ", bestY: " << bestY << ", bestalpha: " << bestalpha << std::endl;
...
void fit_circle_to_points()
{
  ...
  std::cout << "bestX: " << bestX << ", bestY: " << bestY << ", bestradius: " << radius << std::endl;
...
void fit_3D_circle_earth_to_points()
{
  ...
  std::cout << "best radius_3D: " << best_radius_3D << ", best_latitude_3D: " << best_latitude_3D << ", best_longitude_3D: " << best_longitude_3D << std::endl;
...
{
  std::cout << " " << std::endl;
  std::cout << "Draw_3D_circle_planet" << std::endl;
...
class Class_peak_bkgr_cleaner
{
  ...
  Int_t max_cls_index;
  std::vector< std::vector< std::vector< std::vector< std::vector<Int_t> > > > > vec_walking_bins; // stored the full walking path per cluster
  std::vector< std::vector< std::vector< std::vector< std::vector<Int_t> > > > > vec_walking_bins_plot; // stored the full walking path per cluster
  std::vector< std::vector< std::vector<Int_t> > >  vec_init_target_xy_used; // stores the init and target bins per cluster
  std::vector< std::vector< std::vector<Int_t> > >  vec_min_max_x_y_cluster;
  Double_t max_ratio_full_scan;
  Double_t min_max_dist;
  std::vector<Int_t> vec_cluster_size;
  std::vector<Int_t> vec_bins;
  std::vector<Int_t> vec_bins_mod;
...
public:
  ...
  Double_t calc_full_walk_way(std::vector< std::vector<Int_t> > vec_walk_path);
  ...
  std::vector< std::vector< std::vector< std::vector< std::vector<Int_t> > > > > get_vec_walking_bins_plot();
  std::vector< std::vector<Double_t> > get_bkgr_cleaned_points();
  std::vector< std::vector<Double_t> > get_bkgr_points();
  void  clear();
};


std::vector< std::vector<Double_t> > Class_peak_bkgr_cleaner::get_bkgr_cleaned_points()
{
  std::vector< std::vector<Double_t> > vec_2D_peak_points_out;
  std::vector<Double_t> vec_peak_pos;
...
std::vector< std::vector<Double_t> > Class_peak_bkgr_cleaner::get_bkgr_points()
{
  std::vector< std::vector<Double_t> > vec_2D_peak_points_out;
  std::vector<Double_t> vec_peak_pos;
  vec_peak_pos.resize(5);
...
void Class_peak_bkgr_cleaner::add_data(std::vector< std::vector<Double_t> > vec_2D_peak_points_in)
{
  //std::vector< std::vector<Double_t> > vec_2D_peak_points(vec_2D_peak_points_in); // deep copy(?) -> clone all array members
					     ...
					     )
{
  ...
  if(corr_length <= 0.0)
    {
      std::cout << "WARNING in Class_peak_bkgr_cleaner::set_parameters! corr_length <= 0.0, set to 1.0" << std::endl;
      corr_length = 1.0;
    }
  ...
  for(Int_t bin_x = 2; bin_x < max_x_bin; bin_x++) // exclude edge bins
    {
	  ...
	  if(bin_cont > 0.0 && cls_indx == -1)
	      ...
	      std::vector< std::vector<Int_t> > vec_bin_index_nb; // stores the bin indices of the neighbouring bins with positive bin content
		  ...
		  if(!flag_first_bin)
                    {
		      bin_x_use = vec_bin_index_nb[0][0]; // always take the first element in the std::vector, will be deleted at the end
...
void Class_peak_bkgr_cleaner::select_bkgr_clusters()
{
  ...
  std::vector< std::vector<Double_t> > vec_min_max_dist;
  ...
  std::vector<Int_t> vec_flag_bkgr_cluster;
...
//---------------------------------------------------------------------------------------------------------------------
Double_t Class_peak_bkgr_cleaner::calc_full_walk_way(std::vector< std::vector<Int_t> > vec_walk_path)
{
...
{
  std::cout << "Class_peak_bkgr_cleaner::do_full_walk" << std::endl;

  // Create linear 1D list of global bins with bin content > 0
  std::vector< std::vector<Int_t> > vec_list_bin_xy_global; // stores the bin_x and bin_y information of bins with bin_cont > 0
  std::vector<Int_t> vec_list_global_bins; // stores the corresponding global bin
  ...
  for(Int_t i_flip_walk_vector = 0; i_flip_walk_vector < 2; i_flip_walk_vector++)
    {
	  ...
	  for(Int_t i_global_bin_B = 0; i_global_bin_B < (Int_t)vec_list_bin_xy_global[0].size(); i_global_bin_B++) // A->B and B->A might not be the same, do full scan!
	      ...
	      std::vector< std::vector<Int_t> > vec_flag_bin_used_as_close_bin;
	      vec_flag_bin_used_as_close_bin.resize(2);

	      std::vector< std::vector<Int_t> > vec_walking_steps;
    ...
    } // end of flip direction std::vector
  ...
  for(Int_t i_path = 0; i_path < (Int_t)vec_init_target_xy_used[0][0].size(); i_path++)
    {
      ...
      // flip is changing the direction std::vectors of the walking algorithm
      ...
      // Take the minimum for all four combinations, A->B, B->A and with the flipped direction std::vectors for the searching algorithm
  ...
  for(Int_t itt_cluster_separation = 0; itt_cluster_separation < N_itt_cluster_separation; itt_cluster_separation++)
    {
      ...
      std::vector<Int_t> vec_N_cluster;
      for(Int_t bin_x = 1; bin_x <= max_x_bin; bin_x++)
        {
	  for(Int_t bin_y = 1; bin_y <= max_y_bin; bin_y++)
            {
      ...
      std::cout << "end of cluster separation itteration loop" << std::endl;
    } // end of cluster separation itteration loop

}

std::vector< std::vector< std::vector< std::vector< std::vector<Int_t> > > > >  Class_peak_bkgr_cleaner::get_vec_walking_bins_plot()
{
...
class Class_2D_crater_finder
{
  ...
  std::vector< std::vector<Double_t> > vec_radii_peak_positions_quality;
  std::vector< std::vector<Double_t> > vec_radii_peak_positions_radius;
  std::vector< std::vector<Double_t> > vec_radii_peak_positions_radius_merge;
  std::vector< std::vector< std::vector<Double_t> > > vec_radii_partial_ring_angles;
  std::vector< std::vector< std::vector<Double_t> > > vec_radii_partial_ring_angles_radius;
  std::vector< std::vector< std::vector<Double_t> > > vec_radii_partial_ring_angles_merge;
  std::vector< std::vector<Double_t> > vec_radii_peak_positions_radius_merge_copy;
  std::vector< std::vector< std::vector<Double_t> > > vec_radii_partial_ring_angles_merge_copy;
  std::vector<Int_t> index_quality_to_radius;
  std::vector<Int_t> index_radius_to_quality;
  Int_t flag_debug;
  Int_t param_radii;
...
public:
  ...
  std::vector< std::vector<Double_t> > get_radii_peak_positions();
  std::vector< std::vector<Double_t> > get_radii_peak_positions_merged();
  std::vector< std::vector<Double_t> > get_radii_peak_positions_ordered_by_radius();
  std::vector< std::vector< std::vector<Double_t> > > get_radii_partial_ring_angles();
  std::vector< std::vector< std::vector<Double_t> > > get_radii_partial_ring_angles_merged();
...
void Class_2D_crater_finder::order_radii()
{
  ...
  std::vector<Int_t> flag_used_smallest_radius;
...
void Class_2D_crater_finder::find_craters()
{
  ...
  std::vector<Double_t> vec_angle_partial_ring_start_stop;
  vec_angle_partial_ring_start_stop.resize(2); // start, stop

  //----------------------------------------------------------------------
  // Search in rings within inner radius and outer radius
  std::vector<Double_t> vec_quality;
  std::vector< std::vector< std::vector<Double_t> > > vec_partial_rings_angle;
  std::vector<Double_t> vec_points_partial_ring;
  std::vector<Double_t> vec_density;
  std::vector<Double_t> vec_radius;
  std::vector<Double_t> vec_half_width;
  std::vector<Int_t>    vec_N_points;
  std::vector<Double_t> vec_RMS_height;
  std::vector<Double_t> vec_RMS_width;
  std::vector<Double_t> vec_mean_height;
  std::vector<Double_t> vec_mean_width;
  std::vector<Double_t> vec_max_height;
  std::vector<Double_t> vec_min_height;
  std::vector<Double_t> vec_ext_density;
  Int_t i_radius = 0;
  ...
  while(radius_low < max_radius_points && radius_low < max_radius) // scan radius
    {
      ...
      // Make new point std::vector for points within detected ring, add angle information
      std::vector< std::vector<Double_t> > vec_points_ring;
      ...
      std::vector< std::vector<Double_t> > vec_points_ring_ordered_angle;
      vec_points_ring_ordered_angle.resize(4); // x,y,radius,angle
      std::vector<Int_t> flag_used_smallest_angle;
      ...
      std::vector< std::vector<Double_t> > vec_array_angle_start_stop;
  ...
  std::vector<Double_t> vec_exclude_low;
  std::vector<Double_t> vec_exclude_high;
  ...
  std::vector< std::vector<Double_t> > vec_points_in_between;
  vec_points_in_between.resize((Int_t)vec_radii_peak_positions_radius[0].size());
  std::vector< std::vector<Double_t> > vec_quality_in_between;
  ...
  for(Int_t i_2D_radius_qual_cons = 0; i_2D_radius_qual_cons < (Int_t)vec_radii_peak_positions_quality[0].size(); i_2D_radius_qual_cons++)
    {
      if(flag_debug == 2) std::cout << "" << std::endl;
  ...
  std::cout << "" << std::endl;
  std::cout << "-------------------------------" << std::endl;
  std::vector<Double_t> sum_ring_quality;
  std::vector<Double_t> sum_ring_points;
  ...
  for(Int_t i_2D_radius = 0; i_2D_radius < vec_radii_peak_positions_quality[0].size(); i_2D_radius++)
    {
      std::cout << "i_2D_radius: " << i_2D_radius << ", radius: " << vec_radii_peak_positions_quality[0][i_2D_radius]
		...
		<< ", N_points_ext: " << vec_radii_peak_positions_quality[11][i_2D_radius] << std::endl;
  ...
  std::vector<Int_t> vec_index_best_points_ext;
  std::vector<Int_t> vec_flag_best_points_ext;
  ...
  std::cout << "" << std::endl;
  std::vector<Double_t> vec_cons_sum_N_points;
...
void Class_2D_crater_finder::Merge_close_rings(Double_t ring_dist_merge)
{
  ...
  std::vector<Int_t> vec_flag_ring_merged;
  ...
  for(Int_t i_2D_radiusA = 0; i_2D_radiusA < (Int_t)vec_radii_peak_positions_radius_merge_copy[0].size(); i_2D_radiusA++)
    {
	  ...
	  if(fabs((radiusB-halfwidthB) - (radiusA+halfwidthA)) < ring_dist_merge) // Merge rings
	      ...
	      std::vector< std::vector<Double_t> > vec_array_angle_start_stop_B_copy = vec_radii_partial_ring_angles_merge_copy[i_2D_radiusB];
...
std::vector< std::vector<Double_t> > Class_2D_crater_finder::get_radii_peak_positions() // ordered by quality
{
  return vec_radii_peak_positions_quality;
}

std::vector< std::vector<Double_t> > Class_2D_crater_finder::get_radii_peak_positions_merged() // ordered by quality
{
  std::cout << "" << std::endl;
  std::cout << "Merged ring properties" << std::endl;
...
std::vector< std::vector<Double_t> > Class_2D_crater_finder::get_radii_peak_positions_ordered_by_radius()
{
  return vec_radii_peak_positions_radius;
}

std::vector< std::vector< std::vector<Double_t> > > Class_2D_crater_finder::get_radii_partial_ring_angles() // ordered by quality
{
  return vec_radii_partial_ring_angles;
}

std::vector< std::vector< std::vector<Double_t> > > Class_2D_crater_finder::get_radii_partial_ring_angles_merged() // ordered by quality
{
...
//----------------------------------------------------------------------------------------
void Draw_polygon(std::vector< std::vector<Double_t> > vec_points, Int_t color_fill, Int_t color_line, Double_t alpha_fill,
